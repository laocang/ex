...
function openit(target){
	...
	if(rootUrl){
		...
		"&status=E&userId="+$("#userId").val()+"&t="+$("#t").val()+"&m="+$("#m").val()+"&url="+rootUrl;
	}
/*	var data={
		applyKind:applyKind,
		flowId:flowId,
	...
	$.post("flow/getRootUrl",data,function(url){
		...
		if(url!="" && url!=undefined){
			...
			+flowsn+"&flowId="+flowId+"&ratifyId="+ratifyId+"&sender="+sender+"&getDate="+getDate+"&subject="+subject+
			"&status=E&userId="+$("#userId").val()+"&t="+$("#t").val()+"&m="+$("#m").val()+"&url="+url;
		}
	});*/
}
/*
function btngetMoreData(){
	...
	var urlPath = "flow/finalizedworkmore";
	num = parseInt(num)+1;
	var currentPage = num;
...
function searchflow(){
	...
	$.post("flow/finalizedworkmore",params,function(data){
		var json = eval(data);
		 if(json && json.length > 0){
			 //var temp=$("li[name=temp]").first();
			 $("#list").empty();
